# Err0r404G/README.md

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0f172a,100:2563eb&height=220&section=header&text=Sakib%20Sadman&fontSize=48&fontColor=ffffff&animation=fadeIn&fontAlignY=38&desc=Software%20Developer%20%7C%20Web%20%7C%20Desktop%20%7C%20Database&descAlignY=58&descSize=18" width="100%" />

<h1 align="center">Hi There! I'm Sakib Sadman</h1>

<p align="center">
  <a href="https://git.io/typing-svg">
    <img
      src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=24&duration=3000&pause=900&color=2563EB&center=true&vCenter=true&width=800&lines=I+build+clean%2C+responsive+websites;I+develop+desktop+and+database-backed+apps;I+turn+real+ideas+into+polished+digital+products"
      alt="Typing SVG"
    />
  </a>
</p>

<p align="center">
  <img src="https://raw.githubusercontent.com/devSouvik/devSouvik/master/gif3.gif" width="420" alt="Coding" />
</p>

## 👨‍💻 About Me

🚀 Software developer building practical web, desktop, and database-driven solutions  
💼 I work across websites, ecommerce systems, POS apps, and management software  
💡 Mission: Creating useful, clean, responsive, and maintainable digital products  
🔭 Currently studying CSE at American International University-Bangladesh  
💬 Ask me about C#, Java, PHP, Laravel, SQL, ecommerce systems, and desktop apps  
👯 Open to collaborating on web apps, ecommerce, POS, and database-backed projects  
🌐 Portfolio: [www.sakibsadman.site](https://www.sakibsadman.site/)  
📫 GitHub: [Err0r404G](https://github.com/Err0r404G)

## 🌐 Connect With Me

<p align="left">
  <a href="https://www.sakibsadman.site/" target="_blank">
    <img src="https://img.shields.io/badge/Portfolio-2563EB?style=for-the-badge&logo=google-chrome&logoColor=white" />
  </a>
  <a href="https://github.com/Err0r404G" target="_blank">
    <img src="https://img.shields.io/badge/GitHub-111827?style=for-the-badge&logo=github&logoColor=white" />
  </a>
  <a href="https://www.linkedin.com/" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
</p>

## 🚀 Languages, Tools & Technologies

| Category | Technologies |
|---|---|
| Programming Languages | Java, C#, C++, PHP, JavaScript |
| Frontend | HTML, CSS, Bootstrap, Responsive UI |
| Backend | PHP, Laravel, C#, Java |
| Database | MySQL, SQL, MongoDB |
| Frameworks | Laravel, .NET Windows Forms, MVC |
| Desktop Apps | C# WinForms, Java Desktop Applications |
| Tools | Git, GitHub, Visual Studio, VS Code |

## 📌 Featured Projects

### Store Inventory POS
C# desktop inventory and POS management app for stock, sales, refunds, reports, users, and audit logs.

### NexusCommerce
Multi-role marketplace with customers, vendors, delivery managers, admins, carts, coupons, orders, returns, analytics, and earnings.

### Ashvelian Store
Laravel ecommerce management system with role-based workspaces, storefront, checkout, invoices, coupons, inventory, and shipment tracking.

### Restaurant Management
Java desktop app for restaurant order tracking, role-based access, staff communication, and sales management.

### MRT Agro Products BD Limited
Responsive public-facing website for MRT Agro Products BD Limited.

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Err0r404G&show_icons=true&theme=tokyonight&hide_border=true" height="170" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Err0r404G&theme=tokyonight&hide_border=true" height="170" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Err0r404G&layout=compact&theme=tokyonight&hide_border=true" height="170" />
</p>

## 🧭 Portfolio

<p align="center">
  <a href="https://www.sakibsadman.site/">
    <img src="https://img.shields.io/badge/Visit%20My%20Portfolio-sakibsadman.site-2563EB?style=for-the-badge" />
  </a>
</p>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:2563eb,100:0f172a&height=120&section=footer" width="100%" />
